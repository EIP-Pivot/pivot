...
public:
    MixedIndentException(const char *msg, const std::string &line_, size_t linenb_, const std::string &info_)
        : std::exception(msg), line(line_), linenb(linenb_), info(info_)
    {
    }
...
public:
    TokenException(const char *exceptionType, const std::string &token_, size_t linenb_, size_t charnb_,
                   const std::string &exctype_, const std::string &info_)
        : std::exception(exceptionType), token(token_), linenb(linenb_), charnb(charnb_), exctype(exctype_), info(info_)
    {
    }
...
public:
    InvalidException(const char *exceptionType_, const std::string &faulter_, const std::string &error_)
        : std::exception(exceptionType_), exceptionType(exceptionType_), faulter(faulter_), error(error_)
    {
    }
