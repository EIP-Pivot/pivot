...
Node consumeComponent(
    std::vector<Token> &tokens);    // Consume a component token and all following to build a component declaration node
void consumeComponentToken(std::vector<Token> &tokens, Node &result, TokenType expectedType, NodeType fillType,
                           Token &lastToken);    // consume one token
Node consumeSystem(
    std::vector<Token> &tokens);    // Consume a system token and all following to build a system declaration node
void consumeSystemDescriptionToken(std::vector<Token> &tokens, Node &result, TokenType expectedType, NodeType fillType,
                                   Token &lastToken);    // consume one token for declaration
void consumeSystemStatement(std::vector<Token> &tokens, Node &result,
                            Token &lastToken);    // consume an entire statement and append it to children node
void consumeSystemBlock(std::vector<Token> &tokens, Node &result,
                        Token &lastToken);    // consume entire block and append it to children node
void consumeSystemVariable(std::vector<Token> &tokens, Node &result,
                           Token &lastToken);    // consume a variable and append it to children node
void consumeSystemExpression(std::vector<Token> &tokens, Node &result,
                             Token &lastToken);    // consume an expression and append it to children node
void consumeSystemFuncParams(std::vector<Token> &tokens, Node &result,
                             Token &lastToken);    // consume function parameters and append them to children node

void expectSystemToken(
    std::vector<Token> &tokens, TokenType expectedType, Token &lastToken,
    bool consume);    // check that token exists, and is of correct type (and potentially consume it from tokens)
void expectSystemTokenValue(
    std::vector<Token> &tokens, const std::string &expectedValue, Token &lastToken,
    bool consume);    // check that token exists, and is of correct value (and potentially consume it from tokens)
