...
TEST_CASE("Scripting-interpreter-main") {
        ...
        std::cout << "\tPos_x: " <<
std::get<double>(std::get<data::Record>(array1->getValueForEntity(0).value())["pos_x"]) << std::endl; std::cout <<
"\tPos_y: " << std::get<double>(std::get<data::Record>(array1->getValueForEntity(0).value())["pos_y"]) << std::endl;
        std::cout << "\tPos_z: " <<
std::get<double>(std::get<data::Record>(array1->getValueForEntity(0).value())["pos_z"]) << std::endl;
...
std::get<double>(std::get<data::Record>(array1->getValueForEntity(0).value())["pos_z"]) << std::endl;
        ...
        std::cout << "Time to execute computeExpression:\t\t" << engine._totalCompute / 1000000 << "ms\t" <<
(double(engine._totalCompute / 1000000) / totalMs) * 100 << "%" << std::endl; std::cout << "Time to execute std::visit
and ReplaceAll:\t" << engine._totalReplaceall / 1000000 << "ms\t" << (double(engine._totalReplaceall / 1000000) /
totalMs) * 100 << "%" << std::endl; std::cout << "Time to execute parser.compile:\t\t\t" << engine._totalCompile /
1000000 << "ms\t" << (double(engine._totalCompile / 1000000) / totalMs) * 100 << "%" << std::endl; std::cout <<
"--------- End Interpreter main --------" << std::endl;
}
*/
