...
Variable &Stack::findMut(const std::string &name, std::unordered_map<std::string, Variable> &where)
{
    ...
    if (dot == std::string::npos) {
        if (!where.contains(name)) throw InvalidException("UnknownVariable", name, "Unknown variable.");
        return where.at(name);
    }
    if (!where.contains(name.substr(0, dot))) throw InvalidException("UnknownVariable", name, "Unknown variable.");
